 ...
 * Uses environment variables with proper fallbacks, but fails securely if not configured
 */
const getAppUrl = () => {
 ...
 // Server-side: use environment variables in order of preference
  ...
  // Fail securely - no localhost fallback for security
  throw new Error('App URL not configured. Set NEXT_PUBLIC_APP_URL environment variable.')
}
...
export const useAuthStore = create((set, get) => ({
  ...
  signInWithGoogle: async () => {
    ...
    try {
      ...
      // Don't pass redirectTo as query param - it's already in localStorage
      // Query params can cause Supabase URL matching to fail
  ...
  ensureOrganization: async () => {
    ...
    ensureOrgPromise = (async () => {
      try {
        // Org is already created in auth callback via ensure_current_user_org RPC
        // Just fetch organizations here
...
export function AuthProvider({ children }) {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (event, session) => {
        ...
        if (session?.user) {
          ...
          setTimeout(async () => {
            try {
              // Only ensure org if we don't already have one
              const { currentOrganization } = useAuthStore.getState()
              if (!currentOrganization) {
                await store.ensureOrganization();
              }
            } catch (error) {
              console.error("❌ Failed to fetch organization during auth:", error);
            }
          }, 0)
        } else {
  ...
  const logout = async () => {
    ...
    // Redirect to login immediately to avoid rendering protected pages' Access Denied fallback
    try {
